        ...
        for item in data:
            try:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Пользователь с идентификатором {user_id} не существует."
                        )
                    )
                    continue
                ...
                if course_id:
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Курс с идентификатором {course_id} не существует."
                            )
                        )
                        continue
                ...
                if lesson_id:
                    try:
                        ...
                        self.stdout.write(
                            self.style.ERROR(
                                f"Урок с идентификатором {lesson_id} не существует."
                            )
                        )
                        continue
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"Платеж успешно загружен для пользователя {user.id}"
                    )
                )

            except KeyError as e:
                self.stdout.write(
                    self.style.ERROR(f"Отсутствует поле в данных JSON: {e}")
                )
                continue
